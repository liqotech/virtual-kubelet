...
func (pc *PodController) handleProviderError(ctx context.Context, span trace.Span, origErr error, pod *corev1.Pod) {
	...
	case metav1.StatusReasonNotFound:
		err := pc.client.Pods(pod.Namespace).Delete(pod.Name, metav1.NewDeleteOptions(0))
		if err != nil {
			log.G(ctx).WithError(err).Error("setting provider failed, cannot delete local pod %s/%s", pod.Namespace, pod.Name)
	...
	case metav1.StatusReasonServiceUnavailable, metav1.StatusReasonAlreadyExists:
		...
		log.G(ctx).WithError(origErr).Info(origErr.Error())
